...
class _ConversationCreatorState extends State<ConversationCreator> {
  ...
  final Map<String, String> _participants = {}; // userID: userName
  ...
  void _addCurrentUser() {
    ...
    _firebaseService.getUserName(currentUserId).then((userName) {
      setState(() {
        _participants[currentUserId] = userName;
      });
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text("Create Conversation", style: TextStyle(color: Colors.white)),
      ),
      body: Padding(
  ...
  void _createConversation() async {
    if (_participants.isNotEmpty) {
      ...
      try {
        ...
        await _addInitialMessage(conversationRef.id, conversationRef, messageId);
        _firebaseService.updateUserProfile(conversationRef.id);
        _sendMessageOverSocket(conversationRef.id, messageId);
  ...
  Future<void> _addInitialMessage(String conversationId, DocumentReference conversationRef, String messageId) async {
  ...
  Future<void> _sendMessageOverSocket(String conversationId, String messageId) async {
    String senderId = _firebaseService.userId;
    String senderName = await _firebaseService.getUserName(senderId);
    List<String> receiverIds = _participants.keys.where((id) => id != senderId).toList();
    ...
    socketService.socket.emit('msg', messageData);
  }
